...
@Component({ selector: 'app-cy-dummy', template: `` })
...
const getTemplate = (
    ...
    <app-cy-dummy #dummy></app-cy-dummy>
`
...
describe('Dialog API and DialogComponent', () => {
    ...
    it('responds with a custom response code', () => {
        ...
        setupComponent(
            getTemplate('confirm', {
                title: defaultModalTitle,
                buttons: [{ text: 'Abort' }, { text: 'Submit', resCode: 'customResponseCode' }],
            }),
            responseSpy,
        )
        ...
        cy.get(testName('dialog-button'))
            ...
            .click()
            .then(() => {
                expect(responseSpy).to.be.calledOnceWith('customResponseCode')
            })
    })
})
