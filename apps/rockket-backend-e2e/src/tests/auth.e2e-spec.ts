...
describe('Authentication (e2e)', () => {
    ...
    describe('Sign up', () => {
        it('can sign up', () => {
            return signup(app, userFixtures.jonathan).expect(201)
        })
        it('cannot sign up with same email twice', async () => {
            await signup(app, userFixtures.jonathan).expect(201)
            await signup(app, userFixtures.jonathan).expect(409)
        })
        it.todo('complains about insecure passwords')
    })

    describe('Login', () => {
        beforeEach(async () => await signup(app, userFixtures.jonathan).expect(201))

        it('can login', async () => {
            await login(app, userFixtures.jonathan).expect(201)
        })
        it('cannot login with wrong email', async () => {
            await login(app, { ...userFixtures.jonathan, email: 'This is wrong' }).expect(401)
        })
        it('cannot login with wrong password', async () => {
            // @TODO: this seems to return 500 for some reason when not using spread operator
            await login(app, { ...userFixtures.jonathan, password: 'This is wrong' }).expect(401)
        })
    })
        ...
        describe('Password', () => {
            it('can change password', async () => {
                await request(app)
                    .patch('/user/password')
                    .auth(authToken, typeBearer)
                    .send({ password: userFixtures.jonathan.password, newPassword: 'My new password' })
                    .expect(200)
            })
        ...
        describe('Delete Account', () => {
            it('can delete account', async () => {
                await request(app)
                    .delete('/user')
                    .auth(authToken, typeBearer)
                    .send({ password: userFixtures.jonathan.password })
                    .expect(200)
            })
